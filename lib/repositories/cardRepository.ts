...
export async function find() {
  const result = await postgres.query<Card>("SELECT * FROM cards");
  return result.rows;
}

export async function findById(id: number) {
  const result = await postgres.query<Card, [number]>(
...
export async function findByTypeAndEmployeeId(
  ...
  const result = await postgres.query<Card, [TransactionTypes, number]>(
...
export async function findByCardDetails(
  ...
  const result = await postgres.query<Card, [string, string, string]>(
    ` SELECT 
        * 
      FROM cards 
...
export async function remove(id: number) {
  postgres.query<any, [number]>("DELETE FROM cards WHERE id=$1", [id]);
}