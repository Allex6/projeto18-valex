...
import { insert, update, findById, findByTypeAndEmployeeId, TransactionTypes } from './../repositories/cardRepository';
...
async function createCards(apiKey: string, cardData: { employeeId: number, password: string, isVirtual: boolean | undefined, originalCardId: number | undefined, type: TransactionTypes}){
    ...
    const employeeCardWithSameType = await findByTypeAndEmployeeId(type, employeeId);
    if(employeeCardWithSameType) throw errorFactory('limit_reached', 'The employee can only have one card of each type.');
...
async function activateCard(data: { id: number, cvc: string, password: string }){
    ...
    if(cardData.password) throw errorFactory('conflict', 'Your card has already been activated.');
    if(isExpired(cardData.expirationDate)) throw errorFactory('card_expired', 'Your card has expired.');

    const decryptedCVC = cryptr.decrypt(cardData.securityCode);
    console.log('decryptedCVC ', decryptedCVC);

    if(cvc !== decryptedCVC) throw errorFactory('invalid_security_code', 'Could not match the specified security code.');

    const toUpdate = { password: bcrypt.hashSync(password, 12) };
...
async function blockCard(data: { id: number, password: string }){
    ...
    if(cardData.isBlocked) throw errorFactory('conflict', 'Your card has already been blocked.');
    if(isExpired(cardData.expirationDate)) throw errorFactory('card_expired', 'Your card has expired.');
    if(!cardData.password) throw errorFactory('conflict', 'Your card must be activated first.');

    const correctPassword = await bcrypt.compare(data.password, cardData.password);
...
async function unblockCard(data: { id: number, password: string }){
    ...
    if(!cardData.isBlocked) throw errorFactory('conflict', 'Your card is active.');
    if(isExpired(cardData.expirationDate)) throw errorFactory('card_expired', 'Your card has expired.');
    if(!cardData.password) throw errorFactory('conflict', 'Your card must be activated first.');
